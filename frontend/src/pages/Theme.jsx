            ...
            {THEMES.map((t) => (
              <button key={t} onClick={() => changeTheme(t)}
                className={`group flex flex-col items-center gap-1.5 p-2 rounded-lg transition-colors cursor-pointer
                  ${theme === t ? "bg-primary" : "hover:bg-accent"}
                `}>
                ...
                <span className="text-[11px] font-medium truncate w-full text-center text-grey-900">
                  ...
                  <div className="p-4 space-y-4 min-h-[200px] max-h-[200px] overflow-y-auto bg-base-100">
                    {PREVIEW_MESSAGES.map((message) => (
                      <div key={message.id} className={`flex ${message.isSent ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[80%] rounded-xl p-3 shadow-sm ${message.isSent ? "bg-primary text-primary-content" : "bg-secondary"}`}>
